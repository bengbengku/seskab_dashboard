...
class PictureUploader extends Component {
    ...
    if (e.target.files) {
      ...
      for (let i = 0; i < e.target.files.length; i++) {
        if (this.state.lists.length >= this.state.picLimit) {
          this.setState({ warning: "Max Limit!" });
          setTimeout(() => {
    ...
    if (event.dataTransfer.items) {
      ...
      for (let i = 0; i < event.dataTransfer.items.length; i++) {
        ...
        if (event.dataTransfer.items[i].kind === "file") {
          if (
            event.dataTransfer.items[i].type === "image/jpeg" ||
            event.dataTransfer.items[i].type === "image/jpg" ||
            event.dataTransfer.items[i].type === "image/png"
          ) {
            if (this.state.lists.length >= this.state.picLimit) {
              this.setState({ warning: "Max Limit!" });
              setTimeout(() => {
  ...
  renderPicture() {
    if (this.state.lists.length === 0) {
      return <div className="drag-caption">Masukan Foto Anda atau Cari File...</div>;
    } else {
      return (
        <Container>
          <Row>
            {this.state.lists.map((img, i) => (
              <Col className="pic-col" key={i}>
      ...
      <div className="pictureuploader-container">
        ...
        <div className="pictureuploader-info-container">
          ...
          <div className="max-file-info">
            {"Max File"} : {this.state.picLimit} {"Picture(s)"}
          </div>
        </div>
      </div>
    );
